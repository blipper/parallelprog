  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (money < 0)
      ...
      if (threshold(money, coins))
        ...
        val (sameMoney, sameCoins) = parallel({parCountChange(money, coins.tail,threshold)},{parCountChange(money - coins.head, coins,threshold)}) 
        sameMoney + sameCoins
      }
    }
  }
